...
* **Module-Level Session Mocking**: When mocking `Session(engine)` in unit tests for widgets or route helpers that import `from sqlmodel import Session`, always patch the `Session` attribute on the target module (e.g. `patch("app.extensions.<name>.widget.Session", mock_session_cls)`), NOT `patch("sqlmodel.Session")`.
